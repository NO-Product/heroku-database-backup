...
def download_from_destination(file_name):
    ...
    ftp.login(user=current_app.config['FTP_USER'], passwd=current_app.config['FTP_PASS'])
    try:
        ...
        logging.error(f"[download_from_destination] Error downloading file from FTP: {str(e)}")
        return False
    finally:
        if ftp:
            ftp.close()
...
def fetch_destination_filelist():
    ...
    try:
        ...
        logging.error(f"[fetch_destination_filelist] Error fetching file list from FTP: {str(e)}")
        return []
    finally:
        if ftp:
            ftp.close()
...
def delete_file_from_destination(file_name):
    ...
    try:
        ...
        logging.error(f"[delete_file_from_destination] Error deleting file from FTP: {str(e)}")
    finally:
        if ftp:
            ftp.close()
