...
def get_config_vars():
    ...
    try:
        config_vars = {
            ...
            'FTP_PORT': int(os.getenv('FTP_PORT')) if os.getenv('FTP_PORT') else 21,  # Default FTP port is 21, convert to integer if FTP_PORT is set
        ...
        logging.error(f"[get_config_vars] Error fetching configuration variables: {str(e)}")
        return None
