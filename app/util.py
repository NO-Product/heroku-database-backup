...
def compress_backup(backup_file):
    ...
    """
    try:
        with open(backup_file, "rb") as f_in, gzip.open(
            backup_file + ".gz", "wb"
        ) as f_out:
...
def send_email_notification(app_config, subject, message):
    try:
        ...
        if smtp_credentials and recipients:
            server = smtplib.SMTP(
                smtp_credentials["smtp_server"], smtp_credentials["smtp_port"]
            )
            server.starttls()
            server.login(
                smtp_credentials["smtp_username"], smtp_credentials["smtp_password"]
            )
            msg = MIMEMultipart()
        ...
        logging.error(
            f"[send_email_notification] Error sending email notification: {str(e)}"
        )
