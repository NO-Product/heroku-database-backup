...
log_level_str = app.config.get('LOG_LEVEL', 'INFO')  # Default to 'INFO' if LOG_LEVEL is not set
...
log_level = log_level_dict.get(log_level_str, logging.INFO)  # Default to logging.INFO if the log level string is not recognized
...
def manual_backup(db_var, label=None):
    ...
    if db_url is None:
        ...
        sys.exit(1)
    elif isinstance(db_url, str) and db_url.startswith(('postgres://', 'mysql://', 'mssql://', 'oracle://')):
        ...
        backup_filename = f"{label}_{details['database_name']}_{timestamp}" if label else f"{details['database_name']}_{timestamp}"

        backup_file = None
        for attempt in range(3):
            try:
                backup_file = DB_BACKUP_FUNCTIONS[details['database_type']](db_url, backup_filename)
                logging.debug(f"[manual_backup] Backup file created on attempt {attempt+1}: {backup_file}")
                break
            except Exception as e:
                logging.error(f"[manual_backup] Error creating backup on attempt {attempt+1}: {str(e)}")
        ...
        logging.debug(f"[manual_backup] Backup file compressed successfully: {compressed_backup_file}")

        upload_success = False
        try:
            logging.debug(f"[manual_backup] Uploading compressed backup file: {compressed_backup_file}")
            with open(compressed_backup_file, 'rb') as file:
                file_content = file.read()
            if current_app.config['UPLOAD_DESTINATION'] == 'S3':
                upload_success = s3.upload_to_destination(compressed_backup_file, file_content)
            elif current_app.config['UPLOAD_DESTINATION'] == 'FTP':
                upload_success = ftp.upload_to_destination(compressed_backup_file, file_content)
        except Exception as e:
        ...
        # Send email notification
        send_email_notification(app.config, "Backup Failed", "Invalid database connection URL")
        sys.exit(1)
...
def trim_backup_history(db_var, days):
    ...
    if db_url is None:
        ...
        files_to_delete = [file for file in file_list if file.startswith(details['database_name']) and datetime.strptime(file.split('_')[-1], "%Y%m%d%H%M%S") < cutoff_date]
        ...
        for file in files_to_delete:
            ...
            try:
                ...
                logging.error(f"[trim_history] Error deleting file: {file}, Error: {str(e)}")
                failed_deletes.append(file)

        # Send email notification
        if failed_deletes:
            send_email_notification(app.config, "Trim History Completed with Errors", f"Trim history completed. Deleted files: {deleted_files}, Failed deletes: {failed_deletes}")
        else:
            send_email_notification(app.config, "Trim History Completed Successfully", f"Trim history completed. Deleted files: {deleted_files}")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == "manual_backup":
            ...
            print("Usage: python app/backup_manager.py [manual_backup|trim_history] [CONFIG_VAR] [LABEL|DAYS]")
            sys.exit(1)
    else:
